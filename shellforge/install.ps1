...
# Installs the latest (or $env:SHELLFORGE_VERSION-pinned) shellforge release
# binary for Windows. This script only downloads, verifies, and places the
# binary — all of shellforge's actual logic lives in the Go binary itself.
$ErrorActionPreference = "Stop"

$Repo = "mgmaster24/config-gen-tools"
# config-gen-tools holds several tools, each released under its own
# tool-scoped tag (e.g. shellforge/v1.2.3), so releases must be filtered by
# this prefix rather than taking the repo's newest release.
$Tool = "shellforge"
$InstallDir = if ($env:SHELLFORGE_INSTALL_DIR) { $env:SHELLFORGE_INSTALL_DIR } else { "$env:LOCALAPPDATA\shellforge\bin" }
$Version = $env:SHELLFORGE_VERSION
...
# (shellforge/v1.2.3) in $env:SHELLFORGE_VERSION.
if ($Version.StartsWith("$Tool/")) { $Version = $Version.Substring("$Tool/".Length) }
$VersionNum = $Version.TrimStart("v")
$Archive = "shellforge_${VersionNum}_windows_${Arch}.zip"
...
try {
    ...
    Copy-Item -Path (Join-Path $WorkDir "shellforge.exe") -Destination (Join-Path $InstallDir "shellforge.exe") -Force

    Write-Info "Installed shellforge $Version to $InstallDir\shellforge.exe"
    ...
    Write-Info "Run 'shellforge generate' to get started."
}
finally {
