#!/usr/bin/env bash
# Installs the latest (or SHELLFORGE_VERSION-pinned) shellforge release
# binary for this machine's OS/arch. This script only downloads, verifies,
# and places the binary — all of shellforge's actual logic lives in the Go
...
TOOL="shellforge"
INSTALL_DIR="${SHELLFORGE_INSTALL_DIR:-$HOME/.local/bin}"
...
# (shellforge/v1.2.3) in SHELLFORGE_VERSION.
VERSION="${VERSION#"${TOOL}/"}"
VERSION_NUM="${VERSION#v}"
ARCHIVE="shellforge_${VERSION_NUM}_${OS}_${ARCH}.tar.gz"
...
mkdir -p "$INSTALL_DIR"
mv "${WORKDIR}/shellforge" "${INSTALL_DIR}/shellforge"
chmod +x "${INSTALL_DIR}/shellforge"

info "Installed shellforge ${VERSION} to ${INSTALL_DIR}/shellforge"
...
info "Run 'shellforge generate' to get started."
