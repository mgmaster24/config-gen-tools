...
# Installs the latest (or $env:GITFORGE_VERSION-pinned) gitforge release
# binary for Windows. This script only downloads, verifies, and places the
# binary — all of gitforge's actual logic lives in the Go binary itself.
...
$Tool = "gitforge"
$InstallDir = if ($env:GITFORGE_INSTALL_DIR) { $env:GITFORGE_INSTALL_DIR } else { "$env:LOCALAPPDATA\gitforge\bin" }
$Version = $env:GITFORGE_VERSION
...
# (gitforge/v1.2.3) in $env:GITFORGE_VERSION.
if ($Version.StartsWith("$Tool/")) { $Version = $Version.Substring("$Tool/".Length) }
$VersionNum = $Version.TrimStart("v")
$Archive = "gitforge_${VersionNum}_windows_${Arch}.zip"
...
try {
    ...
    Copy-Item -Path (Join-Path $WorkDir "gitforge.exe") -Destination (Join-Path $InstallDir "gitforge.exe") -Force

    Write-Info "Installed gitforge $Version to $InstallDir\gitforge.exe"
    ...
    Write-Info "Run 'gitforge generate' to get started."
}
finally {
